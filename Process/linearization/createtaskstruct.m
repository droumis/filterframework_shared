...
%
% Modifed function from Annabelle Singer
...
%           usevidframe - 1 to load in vid frame and display behind pos data,
%                    default 0.  should be saved in animaldatafolder at
%                    [animalprefix, usevidframe,day], ie 'dwiusevidframe06.mat'.
%                       for mpeg video (nspike):
%                    should contain M.cdata, readout from mpgread, with pic
%                    of track.  call with  M=mpgread('Dwight06.vid',
%                    2400*30, 'truecolor') where 'Dwight06.mpg' is vid
%                    from nspike_extract and image of track is at 2400
%                    seconds
%                       for h264 video (trodes):
%                    should contain M.cdata, readout from vidread, with pic
%                    of track.  call with  M=vidread('Dwight06.vid',
%                    2400*30, 'truecolor') where 'Dwight06.vid' is vid
%                    from nspike_extract and image of track is at 2400
%                    seconds
...
for i = 1:size(index,1)
    ...
    if (day ~= currday)
        ...
        if usevidframe == 1 %load the videoframe
            eval(['vidframe = imread(''',directoryname,fileprefix,'vidframe',dsz, num2str(day), '.png', ''');']); 
        end
    ...
    if (combineepochs & newday) %we are using the same track coordinates for all epochs of the day
        ...
        if usevidframe == 1
            tmplinearcoord = feval(coordProgramHandle,directoryname,posdata, M,cmperpix);
        else
    ...
    if ~(combineepochs)
        ...
        if ~isempty(coordprogram)
            ...
            if usevidframe == 1
                task{day}{epoch}.linearcoord = feval(coordProgramHandle,directoryname,pos{day}{epoch}.data,index,vidframe,cmperpix);
            else
                task{day}{epoch}.linearcoord = feval(coordProgramHandle,directoryname,pos{day}{epoch}.data,index);
            end
