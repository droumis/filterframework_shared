...
function [wellsdio, rewardinfo] = createrewardinfo_wtrack(directoryname,prefix,days,epochs,varargin)
%% This function parses DIO into a rewardinfo struct for the W track.  In rewardinfo{day}{epoch}:
...
% used to be called sj_findwellsfromdio1_wtrack

%example use:  [wellsdio, rewardinfo] = createrewardinfo_wtrack('/opt/data40/mari/Hig','hig',[8 11 15],[2 4 6],[]);
%dio bit numbers not necessary if specified per animal below, but can specify as varargin: 
% (...,'inputdios',[center well0 well2],'outputdios',[center well0 well2]);
% where usually center = well9, 0 = well8, 2 = well10 (but check notes per animal)
...
%     usedios=dionos(li);
    ...
    if exist('inputdios','var') && exist('outputdios','var')
        ...
        %% APOLLO
        %     usedios = [26,25,27];         % INPUTS: center, well 0, well 2
        %     outdios = [10,9,11];          % OUTPUTS: center, well 0, well 2
        %% BRODY
        
          usedios = [23,24,22];           % INPUTS: center, well 0, well 2
          outdios = [7,8,6];           % OUTPUTS: center, well 0, well 2
        
          %except day 13 - see ms_preprocess_all
    ...
    for epoch=epochs,
        ...
        % Output wells
        DIOout1=DIO{day}{epoch}{outdios(1)}; nones_out = length(DIOout1.pulsetimes); % Center well 1: Bits 8/24 out
        DIOout2=DIO{day}{epoch}{outdios(2)}; nzeros_out = length(DIOout2.pulsetimes); % Well 0: Bits 9/25 out
        DIOout3=DIO{day}{epoch}{outdios(3)}; ntwos_out = length(DIOout3.pulsetimes); % Well 2: Bits 10/26 out
...
%         end
    ...
    %welltrigtime_out = output trigger time at that well (only
    ...
    %welltrigtime_out_updated =  input trigger time at that well
