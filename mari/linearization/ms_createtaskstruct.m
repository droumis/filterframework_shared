function ms_createtaskstruct(directoryname,fileprefix,index, coordprogram, varargin)
%
...
%           mpegfr - 1 to load in mpeg frame and display behind pos data,
%                    default 0.  should be saved in animaldatafolder at
%                    [animalprefix, mpgframe,day], ie 'dwimpgframe06.mat'.
%                    should contain M.cdata, readout from mpgread, with pic
%                    of track.  call with  M=mpgread('Dwight06.mpeg',
%                    2400*30, 'truecolor') where 'Dwight06.mpeg' is mpeg
...
for i = 1:size(index,1)
    ...
    if (day ~= currday)
        ...
        if mpegfr == 1
            eval(['load ',directoryname,fileprefix,'mpgframe',dsz, num2str(day), '.mat']); %dsz, num2str(day), after mpgframe
        end
    ...
    if ~(combineepochs)
        ...
        if ~isempty(coordprogram)
            ...
            if mpegfr == 1 %added by ms 9/9/15
                task{day}{epoch}.linearcoord = feval(coordProgramHandle,directoryname,pos{day}{epoch}.data,M,cmperpix);
            else
                task{day}{epoch}.linearcoord = feval(coordProgramHandle,directoryname,pos{day}{epoch}.data);
            end
