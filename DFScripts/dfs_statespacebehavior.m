...
%specify learning type: 'S1Acquisition' or 'Switch' - if running a W track animal, choose 'S1Acquisition'
Ltype = 'S1Acquisition'; 
...
% if runscript
    ...
    % Day Filter
    dayfilter =3:10; %Fabio days 6-13 for S1 acquisition, 14-21 for switch
    
    % Epoch Filter
    epochfilter = 'isequal($type, ''run'') && (isequal($environment, ''MultipleW''))'; %'isequal($type, ''run'') && isequal($environment, ''WTrackA'') && (($exposure>=1) && ($exposure<=10))';  %

iterator = 'singleepochanal';

timefilter = {};

F = createfilter('animal', animals, 'days', dayfilter,'epochs', epochfilter, 'excludetime', timefilter, 'iterator', iterator);
% F = createfilter('animal', animals,'epochs', epochfilter, 'excludetime', timefilter, 'iterator', iterator);

if strcmp(Ltype,'S1Acquisition')
    F = setfilterfunction(F, 'dfams_createbehaveperform', {'rewardinfo','task'},'learningtype',Ltype); 
elseif strcmp(Ltype,'Switch')
    F = setfilterfunction(F, 'dfams_createbehaveperform_switch', {'rewardinfo','oppseqrewardinfo','task'},'learningtype',Ltype); % Include 'oppseqrewardinfo' as an input variable if Ltype == Switch
end
F = runfilter(F);
...
if plotinitialSwitch
figure
hold on
plot(Swtrials(1:SwEpochEdges(8)),S1allProbcorrect(1:SwEpochEdges(8),1),'-r','LineWidth',2)
plot(Swtrials(1:SwEpochEdges(8)),S1allProbcorrect(1:SwEpochEdges(8),2),'--r','LineWidth',1)
plot(Swtrials(1:SwEpochEdges(8)),S1allProbcorrect(1:SwEpochEdges(8),3),'--r','LineWidth',1)
plot(Swtrials(1:SwEpochEdges(8)),S2allProbcorrect(1:SwEpochEdges(8),1),'-b','LineWidth',2)
plot(Swtrials(1:SwEpochEdges(8)),S2allProbcorrect(1:SwEpochEdges(8),2),'--b','LineWidth',1)
plot(Swtrials(1:SwEpochEdges(8)),S2allProbcorrect(1:SwEpochEdges(8),3),'--b','LineWidth',1)
plot(Swtrials(1:SwEpochEdges(8)),repmat(chance,(SwEpochEdges(8)),1),'Color',[0.5 0.5 0.5])
for i = 1:16
