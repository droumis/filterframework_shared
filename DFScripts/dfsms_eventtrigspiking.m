...
% MS updated Jan 2017; originally adapted from Kenny's dfskk_eventtriggeredspiking

%%%%% GENERAL PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
eventconsname = 'dca1ripplescons';
anim = 'Eli';
animals = {'Eliot'};
epochtype = 'run';
...
if runscript
    animals_torun = animals; %{'Fabio', 'Eliot'}; 
    disp(eventconsname)
...
switch eventconsname
    case 'dca1ripplescons'
        ...
        %         epochfilter = epochmaker('all_rip');
        ...
        minvelocity = 0;
        timefilter = { {'kk_getconstimes', '($cons == 1)', 'dca1ripplescons',1,...
    ...
    case 'vsubripplescons' 
        ...
        maxvelocity = 4;                                    
        minvelocity = 0;
        timefilter = { {'kk_getconstimes', '($cons == 1)', 'vsubripplescons',1,...
            'consensus_numtets',consensus_numtets,'minthresh',minthresh,'exclusion_dur',exclusion_dur,'maxvelocity',maxvelocity} };
        ...
        maxvelocity = 4;
        minvelocity = 0;
        timefilter = { {'kk_getconstimes', '($cons == 1)', 'vca3ripplescons',1,...
            'consensus_numtets',consensus_numtets,'minthresh',minthresh,'exclusion_dur',exclusion_dur,'maxvelocity',maxvelocity} };
case 'vca1ripplescons' 
        ...
        maxvelocity = 4;
        minvelocity = 0;
        timefilter = { {'kk_getconstimes', '($cons == 1)', 'vca1ripplescons',1,...
            'consensus_numtets',consensus_numtets,'minthresh',minthresh,'exclusion_dur',exclusion_dur,'maxvelocity',maxvelocity} };
end
 ...
 if runscript
     ...
     %     ca2cellfilter = '(isequal($area, ''CA2'') )';
     %     ca3cellfilter = '(isequal($area, ''CA3''))';
     %     dgcellfilter = '(isequal($area, ''DG'') )';
     %     septcellfilter = '(isequal($area, ''septal''))';
     naccellfilter = '(isequal($area, ''NAc'') && ($numspikes > 100))';  %'(isequal($area, ''NAc''))';
     ...
     for animal = animals_torun
         ...
         if strcmp(epochtype,'sleep')
             epochfilter =  '(isequal($type, ''sleep''))'; % '(isequal($type,''run'') && isequal($environment,''MultipleW''))';% %% || (isequal($type,''run'') && isequal($environment,''MultipleW''))'; %
         ...
         % Filter Creation
         if usedayfilter
             nacf = createfilter('animal', animal,'days',dayfilter,'epochs', epochfilter, 'cells', naccellfilter, 'excludetime', timefilter, 'iterator', iterator);
         else
             % ca1f = createfilter('animal', animal, 'epochs', epochfilter, 'cells', ca1cellfilter, 'excludetime', timefilter, 'iterator', iterator);
             nacf = createfilter('animal', animal,'epochs', epochfilter, 'cells', naccellfilter, 'excludetime', timefilter, 'iterator', iterator);
         end
         ...
         nacf.runscript_params = runscript_params;
         
         % cd(datadir)
         % save(sprintf('Super%s_raw_%s_%s',eventconsname,animal{1}(1:3),date),'ca1f','ca2f','ca3f','dgf','septf','animal','-v7.3');
         % clear ca1f; clear ca2f; clear ca3f; clear dgf; clear septf;
...
if postprocessing
    ...
    for animal = animals_toprocess
        ...
        % initialize output - supervar is useful if running for
        % multiple regions and you want to save the output for future use
%         clear supervar
%         supervar.animalname = animal{1};
%         supervar.runscript_params = [];        
%         supervar.data = {};
%         supervar.data_descript = '{region} / struct array of celloutputs';
...
%             regionscript  % a script of "if" statements with color assignments, etc for various brain regions; see Kenny's
            ...
            % transfer run parameters over
%             if reg ==1
%                 supervar.runscript_params = f.runscript_params;
%             end
...
% load sig modulated cell indices if previously saved
% loaddir = sprintf('/opt/data40/mari/%s/ripplemoddata',anim);
% [Dsigcells, Dcellindices] = find_sigripplemodcells(loaddir,anim,dayfilter,epochtype,'dca1ripplescons');
% [Vsigcells, Vcellindices] = find_sigripplemodcells(loaddir,anim,dayfilter,epochtype,'vca1ripplescons');
...
for c = 1:length(A)
    ...
    smoothing_width = round(smoothing_length*.001/nacf.runscript_params.binsize);   % smoothing width in number of bins
    ...
    if plot_singlerasters
        ...
        if savefigs
            figfilename = sprintf('%sripplespikerasters/%s/%sNAc_%d-%d-%d_%s.fig',datadir,epochtype,anim,A(c).dtc,eventconsname);
            saveas(h,figfilename)
            %to save as eps if cell is significant, after running find_sigripplemodcells:
            if ismember(A(c).dtc,Dcellindices,'rows') || ismember(A(c).dtc,Vcellindices,'rows') % save if sig mod either by D or V
                epsfilename = sprintf('%sripplespikerasters/%s/%sNAc_%d-%d-%d_%s.eps',datadir,epochtype,anim,A(c).dtc,eventconsname);
                print(epsfilename,'-depsc')
            end
        end
        %         close(gcf)
        
    end %end raster plot section for now, 10/3/15
end
...
if calcsig
    ...
    nshuffles = 1000;
        ...
        ripplemod.celltype = naccellfilter;
...
%             if dtc(cellind,1) == day
                ...
                if save_ripplemod
                    modname = sprintf('%s%sripplemod_%s_%s_%s.mat',datadir,anim,epochtype,eventconsname,date);
