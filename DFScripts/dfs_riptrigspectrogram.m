...
% Animal Selection
    ...
    % Day Filter
    dayfilter =6:16; %Fabio days 6-13 for S1 acquisition, 14-21 for switch
    
    % Epoch Filter
    epochfilter =    '(isequal($type, ''sleep''))'; %'isequal($type, ''run'') && (isequal($environment, ''MultipleW''))'; %%'(isequal($type, ''sleep''))'; %%%&& isequal($descript, ''post-allruns''))';%   %%% %'isequal($type, ''run'') && isequal($environment, ''WTrackA'') && (($exposure>=1) && ($exposure<=10))';  %

iterator = 'epocheeganal'; 

tetfilter = '(isequal($area,''dCA3''))'; 
...
F = setfilterfunction(F, 'dfams_calcriptrigspectrogram', {'eeg',eventcons},'eventtype','ripplescons'); 
