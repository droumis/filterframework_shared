function coords = ms_getcoord_6armtrack(directoryname,pos,M,cmperpix)
...
fid = figure;
if exist('M','var')
% image(flipdim(M.cdata,1)) %plot image and flip l/r
image(M.cdata)
set(gca,'YDir','normal') %flips both image and axis u/d to align with pos recon coordinates
hold on
plot((pos(:,2)/cmperpix),(pos(:,3)/cmperpix)) %rescale by cmperpix to align with mpeg: 0.7422 for Fabio, 0.7475 for Eliot; .75682
else
    plot(pos(:,2),pos(:,3));
end
